	...
	private static void processAutoDeploy(Map configMap, BundleContext context)
	{
		...
		if (actionList.size() > 0)
		{
			...
			StartLevel sl = (StartLevel)context
				.getService(context.getServiceReference(StartLevel.class.getName()));
			...
			if (configMap.get(AUTO_DEPLOY_STARTLEVEL_PROPERTY) != null)
			{
				...
				{
					startLevel = Integer
						.parseInt(configMap.get(AUTO_DEPLOY_STARTLEVEL_PROPERTY).toString());
				}
			...
			for (int i = 0; i < jarList.size(); i++)
			{
				...
				Bundle b = (Bundle)installedBundleMap
					.remove(((File)jarList.get(i)).toURI().toString());
